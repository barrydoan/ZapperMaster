package com.temple.zappermaster

class Constants {
    companion object {
        const val ICON_PRESET_UP = "PresetUp"
        const val ICON_OFF = "Off"
        const val ICON_UP = "Up"
        const val ICON_Down = "Down"
        const val ICON_PRESET_DOWN = "presetDown"
        const val ICON_VOL_UP = "VolUp"
        const val ICON_CHANNEL_UP = "ChannelUp"
        const val ICON_VOL_DOWN = "VolDown"
        const val ICON_CHANNEL_DOWN = "ChannelDown"

        const val REMOTE_MODEL_NUMBER = "model_number"
        const val REMOTE_BUTTONS = "buttons"
        const val REMOTE_SHARED = "shared"
        const val REMOTE_TYPE = "type"
        const val REMOTE_MANUFACTURE = "manufacture"
        const val REMOTE_CREATED = "created"

        const val TYPE_ID = "id"
        const val TYPE_NAME = "name"
        const val TYPE_DESCRIPTION = "description"

        const val MANUFACTURE_ID = "id"
        const val MANUFACTURE_NAME = "name"
        const val MANUFACTURE_DESCRIPTION = "description"
    }
}